...
> **History rewritten, 2026-09-14.** At the author's instruction the four `.txt` files were
> purged from the whole history with `git filter-repo --invert-paths`, and `main` was
> force-pushed. **Every commit hash changed.** The hashes quoted on this page and on
> PROVENANCE-01 have been updated to the rewritten ones, so they resolve; any hash written
> down elsewhere before this date will not. A clone taken before the rewrite cannot fast-
> forward and must be re-cloned. The command used was
>
> ```bash
> git bundle create backups/pre-rewrite-20260914/full-history.bundle --all   # 97 MB, verified
> python .../git_filter_repo.py --force --invert-paths \n>   --path Pre.txt --path Pre-00.txt \n>   --path claude-fable/extract/cells_dump.txt \n>   --path claude-fable/extract/cells_index.txt
> git remote add origin https://github.com/once-ere/Pre-Universe_with_Claude.git
> git push --force origin main
> ```
>
> The pre-rewrite history is kept locally in that bundle, which is git-ignored and was not
> published.
...
> removed at his instruction, along with `claude-fable/extract/cells_dump.txt` and
> `cells_index.txt`: no `.txt` file is tracked any more, and the extract is generated by
> `extract_original.wls` instead, as PROVENANCE-00 explains. Where the commands below add or hash them, that is the record of
> what was done at the time; re-running those commands today will simply not find them in
> `git ls-files`. They were in the public history from commit `b3174ea` until they were
...
> replaced three times (`81b010e`, `8ed7066`, `3e8dc4c`) and the shipped version is 208 lines.
...
#
#  Job 3 is why there is no blanket "*.log" rule.  This repository is
#  mostly provenance: 42 of its 118 tracked files are .log, and they are
...
> gained checks — 178 at commit `3e8dc4c`, 182 at `5232d89` — so a clone made today prints
